...
본 저장소의 과제는 각자 개인 브랜치를 생성한 뒤, Pull Request로 제출합니다.

> 절대 `develop` 브랜치에서 직접 수정하지 않습니다.  
> 반드시 개인 브랜치를 생성한 뒤 작업하고, PR을 올려주세요.
...
3. `develop` 브랜치에서 개인 브랜치를 생성합니다.
4. 본인 제출 파일을 `submissions/WeekN/` 아래에 작성합니다.
5. `develop` 브랜치로 Pull Request를 생성합니다.
6. 마감 후 `solution.md` 해설을 확인합니다.
...
과제를 제출할 때는 반드시 `develop` 브랜치에서 새로운 개인 브랜치를 생성합니다.
...
> `develop` 브랜치에서 직접 파일을 수정하지 않습니다.  
...
# submissions/Week1/13기_김기민.md 파일 작성
...
- `develop` 브랜치에서 직접 수정하지 않습니다.
- 반드시 개인 브랜치를 생성한 뒤 작업합니다.
- 제출 파일은 반드시 `submissions/WeekN/` 아래에 작성합니다.
- 파일명은 `기수_이름.md` 형식을 사용합니다.
- PR 대상 브랜치는 반드시 `develop`입니다.
