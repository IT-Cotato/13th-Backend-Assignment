...
sequenceDiagram
    ...
    alt DUPLICATE 또는 SOLD_OUT
        ...
        alt 영향 row = 1 (수량 확보)
            CW->>DB: INSERT INTO coupon_issue(event_id,user_id,'ISSUED')
            alt INSERT 성공
                CW->>DB: COMMIT
...
WHERE id = :eventId
  AND issued_count < total_quantity;
-- → 애플리케이션에서 영향받은 row 수를 확인한다.
--   0이면 INSERT하지 않고, 기존 발급 기록을 조회해
--   '중복 메시지'인지 '수량 소진/이벤트 없음'인지 구분한 뒤 트랜잭션을 종료한다.

-- 2. UPDATE 결과가 1이면 발급 기록 INSERT
INSERT INTO coupon_issue (event_id, user_id, status)
VALUES (:eventId, :userId, 'ISSUED');
-- → UNIQUE(event_id, user_id) 위반이 나면 이 트랜잭션을 ROLLBACK한다.
--   (증가시킨 issued_count도 함께 원복된다.)
...
Kafka는 최소 한 번 전달(at-least-once)을 보장하므로 중복 소비는 정상적으로 발생할 수 있는 상황이다.
```
...
- 성공/실패는 영향받은 row 수(1/0)로 판단한다. 별도 조회나 애플리케이션 재시도가 필요 없다.
```
...
- 조건부 UPDATE도 내부적으로 row 락을 쓰지만, 확인+증가가 한 문장이라 락 보유 구간이
  훨씬 짧다. 비관적 락은 SELECT ~ UPDATE ~ COMMIT 사이 내내 락을 쥔다는 점이 다르다.
```
...
WHERE id = :eventId
  ...
  AND issued_count < total_quantity;
-- 영향받은 row 수가 0이면 그 사이 다른 트랜잭션이 값을 바꾼 것 → 재조회 후 재시도.
```
...
- 처음 읽은 version과 UPDATE 시점의 version이 같을 때만 갱신에 성공한다.
- 다른 트랜잭션이 먼저 값을 바꿨다면 version이 달라져 UPDATE 결과가 0이 되고,
  애플리케이션이 다시 읽어(재조회) 재시도해야 한다.
...
| 장점 | 구현 단순, 추가 조회·재시도 없이 초과 방지, 락 보유 구간 짧음 | 순서·정합성이 직관적, 이해하기 쉬움 | 충돌이 적으면 락 대기 없이 처리량 유리 |
...
```
1. 짧고 원자적이다.
   확인+증가가 한 SQL이라 락 보유 구간이 가장 짧다. 선착순처럼 같은 row에 몰리는 상황에서
   트랜잭션을 짧게 유지해 병목의 영향을 줄일 수 있다.
