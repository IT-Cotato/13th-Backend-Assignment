...
| --- | ---: | --- | --- |
| Notification (2개) | 2 | Partition 4개는 두 Consumer가 나눔 | 없음 |
| MyPage (4개) | 4 | Partition 2개는 네 Consumer가 나눔 | 없음 |
| Analytics (1개) | 1 | Partition 6개를 한 Consumer가 처리 | 없음 |
...
| --- | --- | --- | --- |
| `schemaVersion` | 이벤트 JSON 구조 | Consumer와 Payload 호환성 | 선택 필드 추가 후 v2 |
...
| --- | --- | --- | --- |
| Coupon `coupon_issue` | 예 | 원본 복구 절차 필요 | 발급 원장 |
...
수정 구조는 1절의 전체 다이어그램과 4·5절의 멱등 트랜잭션 구조를 따른다. 쿠폰 DB의 최종 성공, 각 Group의 소비 기회, MyPage·Analytics의 중복 방지는 보장한다. Kafka 물리적 Exactly-Once와 외부 SMS Exactly-Once는 보장하지 않으며(SMS는 Provider 멱등 키가 있을 때 조건부), 발급 직후 MyPage의 즉시 일치도 보장하지 않는다.

## 핵심 질문 답

1. 서로 다른 Group이어야 각 서비스가 같은 이벤트를 독립적으로 모두 처리한다.
2. `CouponIssued`는 성공 후의 사실이라 Consumer가 같은 행동을 다시 명령받지 않는다.
3. 발급 여부의 최종 기준은 Coupon Service의 `coupon_issue`다.
4. 재발행은 같은 논리 이벤트를 Kafka의 다른 위치에 다시 저장하기 때문이다.
5. 처리 기록과 비즈니스 변경이 함께 성공·실패해야 영구 누락을 막는다.
6. 원장 Commit과 Read Model 반영은 비동기 단계라 잠시 차이가 난다.
7. 버전 값은 순서를 알려 줄 뿐, 늦은 전달을 자동으로 차단하지는 않는다.
8. 통계·알림은 각 상태 전이 자체가 의미 있으므로 과거 버전도 필요할 수 있다.
9. 파생 데이터의 버그·스키마 변경·유실을 원본에서 복구할 수 있어야 한다.
10. 알림 실패는 후속 작업 실패일 뿐, 이미 확정된 발급을 다시 하면 중복 발급 위험이 있다.
