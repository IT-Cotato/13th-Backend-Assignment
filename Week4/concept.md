...
Consumer를 통해 DB에 도달한 발급 요청에 대해 DB 제약 조건과 트랜잭션으로 사용자 중복 발급과 수량 초과를 방어한다.
...
3. DB 저장 후 Kafka Offset 반영 전에 Consumer가 종료될 수 있다.
...
→ 수정된 행이 없음
→ 남은 쿠폰 수량을 확보하지 못함
→ 존재하지 않는 이벤트, 쿠폰 수량 소진, 이미 처리된 중복 메시지 등을 구분해야 함
```

다만 결과가 0이라고 무조건 품절인 것은 아니다. 이벤트가 존재하지 않는 경우에도 0이 반환된다.

이벤트 상태나 기간까지 UPDATE 조건에 포함한다면 `READY`, `CLOSED`, 기간 밖인 경우에도 0이 반환될 수 있다. 사용자에게 정확한 실패 원인을 알려야 한다면 별도 조회로 원인을 구분해야 한다.

이번 구조에서는 API와 Redis에서 이벤트 유효성을 먼저 확인하고, Consumer는 전달받은 요청의 최종 수량을 방어한다고 가정한다.
...
DB 처리가 최종 성공했거나 이미 동일한 발급 기록이 존재한다는 사실을 확인한 다음에만 메시지 처리를 완료해야 한다.

Offset 처리 방식은 Consumer 설정에 따라 자동 커밋, 수동 커밋, 프레임워크의 acknowledgment 등으로 달라질 수 있다. 중요한 원칙은 DB 처리 완료보다 메시지 처리가 먼저 완료되어서는 안 된다는 것이다.
