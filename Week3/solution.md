...
```
1. SCARD와 SADD를 따로 실행한다.

문제:
Race Condition으로 수량 초과
...
```
3. Redis SUCCESS를 최종 발급 성공으로 응답한다.

문제:
Kafka 또는 DB 실패 시 사용자에게 잘못된 성공 안내
...
```
4. DUPLICATE와 SOLD_OUT도 Kafka에 발행한다.
...
```
5. Kafka 발행 실패 후 Redis 상태를 그대로 둔다.

문제:
유령 통과자 발생

개선:
실패 기록, 재시도, 필요한 경우 SREM
```

```
6. SADD와 SISMEMBER의 userId 형식이 다르다.
...
```
7. DB UNIQUE만으로 전체 수량을 지키려 한다.

문제:
서로 다른 사용자 1,001명을 막지 못함
...
```
8. API Server가 서로 다른 limit을 Lua에 전달한다.

문제:
서버별 캐시 차이로 판정 기준 불일치

개선:
Redis metadata를 공통 기준으로 사용
```

```
9. 과거 cleanupAt을 EXPIREAT에 전달한다.

문제:
Key가 즉시 삭제되어 SCARD가 계속 0으로 보일 수 있음
...
```
10. 일반 캐시 Redis에서 선착순 Key를 함께 관리한다.

문제:
Eviction으로 Key가 삭제될 수 있음

개선:
전용 Redis + noeviction + 메모리 모니터링
```

```
11. Lua Script가 자동 롤백된다고 생각한다.

문제:
중간 오류 전의 쓰기가 남을 수 있음
...
```
12. Kafka Timeout을 무조건 실패로 보고 SREM한다.

문제:
실제로 Kafka에 저장된 메시지가 있을 수 있음
...
```
13. UNIQUE 위반을 무조건 FAILED로 처리한다.

문제:
Kafka 재처리로 이미 성공한 요청일 수 있음

개선:
requestId로 기존 처리 결과 확인
```

```
14. Outbox가 Redis와 Kafka를 원자적으로 묶어준다고 생각한다.

문제:
Redis와 RDB는 서로 다른 저장소

개선:
Outbox는 DB 변경과 결과 이벤트 발행에 적용
```
