...
Outbox는 정상 경로보다 장애 경로를 검증하는 것이 중요하다.
...
### 26-2. DB Commit 후 Publisher 실행 전 종료

```
발급 Transaction Commit
Publisher 실행 전 애플리케이션 종료
...
### 26-3. Claim 후 Kafka 발행 전 종료

```
PENDING → PROCESSING Commit
Kafka 발행 전 Publisher 종료

기대 결과
→ Lease 만료
→ PENDING 복구 또는 재Claim
→ Kafka 발행
```

### 26-4. Kafka 발행 성공 후 상태 변경 전 종료

```
Kafka Record 저장 성공
Outbox PUBLISHED 변경 전 종료

기대 결과
→ 동일 eventMessageId 재발행 가능
→ Consumer 비즈니스 결과는 한 번만 반영
```

### 26-5. 두 Publisher의 동시 Claim

```
Publisher A와 B가 동시에 Polling

기대 결과
→ 같은 PENDING 행을 동시에 Claim하지 않음
→ 서로 다른 행을 나누어 처리
```

### 26-6. 오래된 Publisher의 상태 변경

```
Publisher A Lease 만료
Publisher B가 새 Token으로 회수
Publisher A가 뒤늦게 상태 변경 시도
...
### 26-7. Consumer DB Commit 후 Offset Commit 전 종료

```
processed_event와 조회 모델 Commit
Offset Commit 전 Consumer 종료

기대 결과
→ 이벤트 재전달
→ processed_event 중복 확인
→ 조회 모델 중복 변경 없음
```
...
```
같은 aggregateId에 대해
이벤트 A(aggregateVersion=1)의 발행을 인위적으로 실패시키고
이벤트 B(aggregateVersion=2)를 먼저 발행

기대 결과
→ Consumer가 B를 먼저 수신
→ 이후 도착한 A를 aggregateVersion 비교로 무시
→ 최종 상태는 B 기준
```

### 26-9. `deduplication_key` 충돌

```
같은 aggregateId에 대해
같은 eventType의 Outbox INSERT를 강제로 두 번 시도
...
### 26-10. Redis와 DB의 divergence

```
Redis 사용자 SET에서
이미 발급받은 사용자의 Member만 인위적으로 제거하고 재요청

기대 결과
→ Redis SUCCESS
→ Lua Script가 사용자를 Set에 다시 추가
→ DB UNIQUE 위반
→ request FAILED / DUPLICATE_USER
→ 현재 요청의 CouponIssued Outbox 없음
→ 기존 coupon_issue가 정상적으로 존재함을 확인
→ Redis Set과 DB 발급 기록이 다시 수렴하는지 확인
→ 경고 로그와 알람 발생
```

추가로 Redis limit를 DB `total_quantity`보다 크게 설정하거나, Redis Set을 초기화한 뒤 신규 사용자를 요청하게 해 DB `SOLD_OUT` 경로도 검증한다.
...
### 27-1. 발급 트랜잭션 안에서 Kafka를 직접 호출한다

DB와 Kafka 사이의 원자성이 생기지 않는다.

### 27-2. `AFTER_COMMIT`이면 이벤트가 유실되지 않는다고 생각한다

Commit 이후 프로세스 장애와 비동기 발행 실패를 복구할 영속 기록이 없다.

### 27-3. `KafkaTemplate.send()` 반환 직후 PUBLISHED로 변경한다

메서드 호출과 Broker ACK 성공은 다르다.

### 27-4. ACK 전에 PUBLISHED로 변경한다

DB에는 완료로 남지만 Kafka에는 Record가 없을 수 있다.

### 27-5. Partition Key만 맞추면 이벤트 순서가 보장된다고 생각한다

Partition Key는 같은 Partition으로 보낸다는 것만 보장한다.

`SKIP LOCKED`, Backoff 재시도, 병렬 Publisher가 발행 순서를 뒤집는다.

`occurredAt`은 추적 정보로 사용하고,
Consumer는 `aggregateVersion`으로 순서 역전을 방어해야 한다.

### 27-6. Kafka Producer 콜백 스레드에서 DB 작업을 한다

`whenComplete`는 Producer의 Sender 스레드에서 실행된다.

거기서 DB 트랜잭션을 수행하면 Producer 전체 전송이 멈춘다.

`whenCompleteAsync`와 전용 Executor를 사용해야 한다.

### 27-7. 동기 모델과 비동기 모델을 섞어 구현한다

Lease 시간 계산 기준과 스레드 모델이 서로 다르다.

한 가지 모델을 선택하고 그 기준으로 Lease를 계산해야 한다.

### 27-8. 단일 Publisher이므로 Lease가 필요 없다고 생각한다

Claim Commit 직후 단일 Publisher가 종료되어도 `PROCESSING`이 고착된다.

### 27-9. 만료된 PROCESSING 조회 로직이 없다

`WHERE status = 'PENDING'`만 조회하면 만료된 `PROCESSING`은 복구되지 않는다.

### 27-10. 성공 UPDATE에만 Claim Token을 확인한다

실패 후 PENDING 복구에도 Token 조건이 없으면 오래된 Publisher가 최신 상태를 덮어쓸 수 있다.

### 27-11. Outbox INSERT 충돌을 `ON CONFLICT DO NOTHING`으로 넘긴다

기존 행과 현재 이벤트의 동일성을 확인하지 않은 채 불일치나 Key 생성 버그를 숨길 수 있다.

현재 설계에서는 Rollback하고 알람을 발생시켜야 한다.

### 27-12. Kafka 발행 실패 시 쿠폰을 다시 발급한다

DB 발급이 Commit되었다면 Outbox 이벤트만 재발행해야 한다.

### 27-13. Producer Idempotence가 Outbox 중복까지 제거한다고 생각한다

Producer 내부 재시도 중복은 줄일 수 있지만 프로세스 재시작 후 애플리케이션 재발행까지 제거하지는 않는다.

### 27-14. Consumer Offset을 멱등성 Key로 사용한다

같은 논리 이벤트가 재발행되면 Offset은 달라진다.

Consumer 멱등성은 `eventMessageId`를 사용한다.

### 27-15. Publisher가 현재 비즈니스 테이블을 다시 조회해 Payload를 만든다

Outbox 생성 시점의 사실과 발행 시점의 현재 상태가 섞일 수 있다.

저장된 Payload를 그대로 발행한다.

### 27-16. `processed_event`를 짧은 기간만 보관한다

Kafka Topic 보관 기간보다 짧으면 오래된 Record 재전달 시 중복 방어가 뚫린다.

### 27-17. 최종 실패로 확정하면 시스템 상태도 정리된다고 생각한다

`SOLD_OUT`이나 `DUPLICATE_USER`로 확정하는 것은 현재 요청의 사용자 응답을 결정하는 것이다.

Redis 상태는 DB의 기존 발급 기록과 현재 Set 상태를 확인한 뒤 원인에 맞게 보정해야 한다.

### 27-18. `processed_event`만으로 외부 알림도 정확히 한 번이라고 생각한다

외부 API 호출은 로컬 DB 트랜잭션에 포함되지 않는다.

알림 서비스 내부의 추가적인 Outbox 또는 Idempotency가 필요하다.
